...
**Receipt build identity (who confirmed it).** Every receipt also carries the reporting pusher's **build identity** — the same module-graph mtime stamp its transport marker holds (newest across the entry file *and* its `hooks/` imports, because the stale part is as likely `submit.mjs` as the entrypoint). A `delivery:"confirmed"` whose reporter is **behind the on-disk pusher source** stays confirmed — the command demonstrably ran — but the result carries a `warning` (DM form) or `notes` (room form) naming the stale code, so a confirm issued by pre-upgrade verification logic never reads identically to a current one. A receipt with **no stamp** is annotated as unknown provenance, never treated as fresh — absence is not exemption, same ruling as `doctor`'s freshness checks. This is honesty, not security: a lying pusher defeats it, exactly like `report_transport` — the value is diagnosability.
