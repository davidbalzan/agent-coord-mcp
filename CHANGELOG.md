...
### Fixed (P2 — the board parser accepted a column it could not round-trip)
- **A lane row of the wrong arity is refused loudly, never silently reshaped.** `BoardRow` is a fixed 5-tuple, but the parser destructured *any* `| … |` row into it — so `export_work` silently narrowed 6-column rows and widened 4-column ones, on a tool that advertises byte-identical round-trips (hit live 2026-07-29: #38's `Pane` column was dropped on write-back, putting main at 224/226 for two commits). A wrong-arity row now becomes a `MalformedRow`: refused a record (`boardRowsOf` never returns it), preserved byte-exactly on render, and named loudly — line number, actual vs expected column count, and the quoted row — via `import_work` (per-doc `issues` + a top-level warning), `list_work` (`issues`, so a missing lane doesn't read as a lane that doesn't exist), and `export_work` (per-file `issues`, since `identical:true` is not the same as healthy). Design ruling recorded: the schema is *not* widened — a parser that refuses unknown arity keeps the schema decision with the board owner instead of absorbing whatever a docs commit adds.
- **Deliberate edges, pinned by test:** an empty trailing cell (`| … | |`) is still five columns and still a legitimate row (a strictness fix must not break the common case); one bad row refuses the ROW, never the document — records before and after it survive, because a single typo taking down the whole board's import would be worse than the silent drop this replaces; `BOARD_ARITY` and `boardCells` are pinned against drifting apart.
...
### Added (P2 — receipts carry the build identity of the pusher that confirmed them)
- **A delivery receipt names the code that did the confirming.** Both pushers stamp their build identity on every receipt they write — the same module-graph mtime basis their transport marker carries (newest across the entry file *and* the `hooks/` imports, since the stale part is as likely `submit.mjs` as the entrypoint — the #28 lesson, now carried across the wire; the remote pusher's stamp was still entry-file-only and has been upgraded to the graph basis for its marker too). `report_receipt` accepts and stores the stamp; it never defaults one — assume-fresh is the twin of the assume-success it already refuses.
- **`deliveryOutcome` annotates a confirmed receipt from an outdated pusher.** The verdict is never downgraded — the command demonstrably ran — but a confirm issued by verification logic older than the on-disk pusher source now says so (`warning` on the DM form, `notes` on the room form), instead of reading identically to a current one. An absent stamp reads as **unknown provenance, never fresh** (absence is not exemption, same single ruling as doctor's freshness checks), and the absence note is issued before the on-disk comparison so an unstattable hooks dir cannot silence it. Honesty, not security: a lying pusher defeats it, exactly like `report_transport` — the value is that a rollout gap in the *verification* path is visible at the moment it matters, on the receipt.
...
Two of these were found only because a fix was rolled out and then *checked in production* rather than trusted from a green suite.

### Fixed (P1 — freshness checks could not see the code they guard)
- **Pusher freshness covers the loaded module graph, not one file.** The transport marker stamped the mtime of `hooks/tmux-pusher.mjs` alone, but the pusher imports `submit.mjs`, where both control-submit fixes live. A pusher spawned before either fix stamped a *matching* `scriptMtime` whenever the entry file happened to be untouched — so `stale-pusher-script` reported `ok` on a pusher running exactly the code the fix replaced. Both the stamp and the check now derive from the newest mtime across `hooks/*.mjs`, via one shared helper so the two sides cannot drift. It deliberately over-covers non-imports: a loud spurious re-attach beats a silent false green.
- **The server's build identity is sampled at load, stamped on markers, and checked.** Merging a fix does not deploy it — `dist/` may be unbuilt, and a running MCP server holds pre-merge code in memory regardless. `server-build-drift` compares the load-time sample against on-disk `dist`; `dist-behind-source` compares `dist` against `src`. They catch different things: the first is a *process* behind its dist and a restart fixes it; the second is the *disk* behind itself, fires with zero live sessions, and no restart can fix it. A stale server cannot self-report by definition, so markers now carry `serverBuildMtime` provenance and a restarted session can name every marker a stale one stamped — the absence of the check is itself the tell.
- **A marker rewrite merges over the existing marker instead of rebuilding it.** An earlier from-scratch rewrite silently dropped `scriptMtime` and disabled the stale-pusher check for every local pusher. A marker field a rewrite can drop is a health check that can be switched off by accident; the merge contract is now a tested seam, pinned with an unknown `futureField` so it covers fields that do not exist yet.
- **Absence is not exemption.** A live local marker missing `scriptMtime` (or `serverBuildMtime`) reported `ok` — the check silently exempted precisely the markers it exists to police, and the field's own writer was what dropped it. Both now report as unverifiable, flipped in a single commit so the two checks can never disagree about what a missing stamp means. Remote markers keep their documented skip: you cannot stat another host. The transition is deliberately correct-and-loud — every pre-provenance marker warns at once, cleared by a re-attach, because that burst is the only external view of which sessions still run pre-provenance servers.
- **A pattern kill can be scoped to one agent.** `attach_agent` spawns the pusher with `--agent <id>` in argv (inert to the pusher; env stays authoritative). A `pkill -f "tmux-pusher.mjs"` during one agent's cleanup previously matched every live pusher on the bus and silently detached all of them.

### Fixed (P1 — control commands could never reach an idle agent)
- **The draft guard reads styling: a ghost suggestion is chrome, not a draft.** `send_command` correctly refuses to paste onto unsent text — but Claude Code renders a dim, session-derived *suggested prompt* inside an otherwise **empty** input box, and the guard captured with `capture-pane -p` (no `-e`), discarding the very style layer that distinguishes chrome from content. Every control command to an idle agent was refused against an empty input. Ghost text is now partitioned out by SGR state: dim-attributed spans are chrome, everything else — **including any styling we do not recognise** — is treated as a real draft, because a false refusal costs one command while a false delivery types into someone's genuine unsent text. Verified live end to end, on the hardest available case: an idle pane whose ghost suggestion *was* `/compact`. Delivered, confirmed, and the pane compacted.
- **The submit verifier is no longer fooled by a ghost containing the payload.** Same seam, same capture: a suggestion echoing the just-run command read as "still in the input", and the retry path would have typed extra Enters into a live session.
- **A content regex cannot solve this.** A `PLACEHOLDER_PATTERN` already existed; a session-derived suggestion has an unbounded value space, so the discriminator has to be styling. The SGR assumption is pinned in one named seam (`AGENT_COORD_GHOST_TEXT_SGR`) and commented as a per-harness rendering detail, not a protocol guarantee — other supported CLIs may style suggestions differently or not at all. Refusals now quote the *styled* capture and name the rule that fired, so the next false positive self-diagnoses.

### Docs
- **Clarified remote-node onboarding in the README.** Added a concrete Tailscale-bind example (`AGENT_COORD_HTTP_PORT` + `AGENT_COORD_BIND` + `AGENT_COORD_INSECURE=1` + minted token) and a new "Joining from another machine" section that splits client-side from operator-side setup: a pure MCP client (Claude Code) needs **nothing installed** — just the HTTP config entry — while the global `npm i -g` is only for the optional `coord-pusher` bin. Called out the common trap that installing the package globally does **not** connect you to a remote bus (the stdio bin is a separate local file-backed bus). Reconciled two stale spots that still said the server "warns" on a non-loopback bind — it refuses (fail-closed) since 0.15.0.
...
### Fixed (P1 — the gate failed ~25% of green runs)
- **The doctor suite no longer cancels itself on a missed `exit` event.** In the wedged-pusher test, `doctor({fix:true})` SIGTERMs the dummy pusher MID-CALL, so the child's `exit` event could fire while doctor's remaining checks were still awaiting; the `await new Promise((r) => dummy.once("exit", r))` attached afterwards then waited forever on an event that had already happened, and with the child dead nothing kept the event loop alive — node reported "Promise resolution is still pending but the event loop has already resolved" and cancelled the rest of the file (4 tests, ~20–25% of runs, reproduced on clean main back past #28). `check-test-count` caught every occurrence correctly — which meant a quarter of green work failed its gate on a coin flip and read as the author's regression. Diagnosed by instrumentation, not inference: a preload tracer on every process in the test tree showed the dummy taking doctor's SIGTERM (+52ms, correct) and the test process taking **no signal at all**, just `beforeExit` 6ms later. The wait is now guarded (`exitCode`/`signalCode` are assigned before `exit` is emitted, so the guard cannot itself race), and a regression test reproduces the race deterministically — kill the child, wait until `exit` has demonstrably fired, then wait again — failing loudly by name (verified by mutation) instead of draining the loop.
...
The case that justifies the phase, measured on a message that went through `send_message` and hit disk: `"Hey — BLOCKER: prod is down"` classified **routine** under prefix matching, because matching requires byte 0. As a typed record it wakes the pane.

### Added (Phase 8 Tasks 1–3 — the typed envelope)
- **`MessageRecord {type, payload?, cites?}` on every message.** Nine types covering the prefixes already in use, plus `verdict` — a gate PASS/FAIL, which had no prefix and therefore could not be routed at all. Payload shapes are pinned per type: a `decision` carries the playbook's five fields (title / context / options / recommendation / ifNoAction) or is rejected, because a packet missing two of them renders as a truncated packet, which is worse than none.
- **Citations are typed.** `cites: [{kind: "pr"|"file"|"commit"|"url", ref}]`, and a `done` record without a `pr` citation is refused at send time — presence and shape only, no network call in the send path. An uncited DONE is an unverifiable claim.
- **Typed records render to the text layout the fleet already reads.** `renderRecord` reproduces the decision-packet layout byte-for-byte so the existing UI card parser keeps working while agents migrate, and a record renders only to FILL an absent `text` — the author's wording is never overwritten.
- **Typed is not authenticated.** `record` is caller-supplied exactly like `from`: `scope` still resolves the sender against the registry, `done` still depends on the recipient being a gate runner, and nothing in a record can set the server-only `urgent` flag.

### Added (Phase 8 Task 2 — tiering reads the record, as a floor)
- **Delivery tier comes from `record.type` first, prefix second — and the record can only RAISE a tier, never lower one.** An earlier revision let the record win outright, on the reasoning that text is rendered from the record so the two agree by construction. That is false when both are supplied: `text` wins for rendering, so `{text: "BLOCKER: prod down", record: {type: "fyi"}}` displayed a BLOCKER in the pane and delivered it routine, with the reader unable to see why. In v1 a byte-0 `BLOCKER:` always woke the pane, so that was a regression in a safety property, reachable by ordinary relay — an aide forwarding a worker's body under its own `fyi` would have buried that worker's blocker. The floor is monotone, so there is nothing for two sources to disagree about; it also fails safe when an old pusher meets an unknown future record type.

### Fixed (P1b — the submit verifier reported failure on a successful submit)
- **`stillInInput` now reads the input line, not a tail window.** The verifier added in #21 squashed the last 20 lines of the whole pane and asked whether the command appeared anywhere in them — but the TUI echoes a submitted command into the transcript, directly above the prompt box. So a command that ran was reported as `submitted:false, "still in the input… it was typed but did not run"` while the pane read "Compacting conversation… 28%" (David, live). It now locates the prompt line (`AGENT_COORD_PROMPT_PATTERN`) and tests only that. Reproduced live on a pane with populated scrollback: on the identical capture, the old check says "still in the input", the new one says submitted.
- **A retry only ever follows a fresh reading**, so Enter is never sent on a stale verdict — under the old check that fired on every successful submit, pressing Enter three times into a live session. An unreadable pane, or one with no line matching the prompt pattern, stays UNKNOWN and reports why; it is never upgraded to a confirmation.
- **`check-test-count` compares PASS, not `# tests`.** The reporter-gap sighting (186 tests / 182 pass / 0 fail) would have slipped past the original comparison, since `# tests` read the full 186. Totals that do not add up are now reported as a reporter gap in their own right, rather than as a missing test file.
...
### Added (Phase 8 Task 6 — records travel structurally)
- **A multi-line record is delivered as ONE attributed line.** `injectLine` emits a single `[tag HH:MM from]` header per message, so a rendered 7-line decision packet arrived with six unattributed lines a parser could not assign to a sender or a room. A typed record now delivers as a one-line digest plus a retrieval handle; pane rendering stops being a wire format, which is the seam into Phase 9.
- **`retrieve_message({agentId, id})`** expands the handle into the full typed record, reading the live file and falling through to the append-only `archive/` on a miss — no cache, no TTL, nothing to expire. Authority is by construction rather than by check: it only ever opens the caller's own inbox and rooms the caller belongs to, so a handle for a message delivered elsewhere is not refused, it is simply not found.
- **Record-less messages are untouched**, including multi-line ones — they still render exactly as before. A multi-line record with no id is deliberately NOT digested: issuing a handle that cannot be redeemed would destroy content with no way back.

### Fixed (defects found while building the phase)
- **A stored `kind` overwrote the pushers' channel tag.** Both pushers built their render object as `{kind: label, ...m}`, spreading the stored message last — so `Message.kind` (retention weight) overwrote the synthetic channel tag that `injectLine` and `classifyTier` read. A room post tagged `kind: "decision"`, which the README recommends for GOs and verdicts, rendered as `[decision …]` instead of `[#general …]`, corrupting the field harnesses parse the room out of. The process-local half is now named `tag`.
- **The stale-pusher check had never worked.** `hooks/tmux-pusher.mjs` rewrote its own transport marker without `scriptMtime`, clobbering the value `attach_agent` stamps — and `doctor` SKIPS markers lacking that field, so the check that exists to catch a daemon running pre-upgrade code (silently dropping control commands) could not fire for any local pusher since it shipped. A health check whose precondition is written by the thing it monitors cannot fail loudly.
- **`doctor --fix` could SIGTERM an unrelated process.** The wedged-pusher reaper signalled any pid-alive/pane-dead marker's pid without verifying the process was actually a pusher; on a recycled pid it would kill something else on the machine. The marker is still cleared unconditionally — the pane is gone either way — but the signal now requires positive identification, and fails closed when it cannot verify.
## [0.17.0] — Unreleased
